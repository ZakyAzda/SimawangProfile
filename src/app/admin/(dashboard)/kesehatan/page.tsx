...
export default function AdminKesehatanPage() {
  const [items, setItems] = useState<DataKesehatan[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [formData, setFormData] = useState({ jorong: "", jumlahStunting: 0 });

  const fetchData = async () => {
    try {
      const res = await fetch("/api/data-kesehatan");
  ...
  const openCreateModal = () => {
    setFormData({ jorong: "", jumlahStunting: 0 });
    setEditingId(null);
    setIsModalOpen(true);
  };

  const openEditModal = (item: DataKesehatan) => {
    setFormData({ jorong: item.jorong, jumlahStunting: item.jumlahStunting });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const url = editingId ? `/api/data-kesehatan/${editingId}` : "/api/data-kesehatan";
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          jumlahStunting: Number(formData.jumlahStunting)
        })
      });
      if (res.ok) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Hapus data kesehatan ini?")) return;
    try {
      const res = await fetch(`/api/data-kesehatan/${id}`, { method: "DELETE" });
      if (res.ok) fetchData();
    } catch (e) {
      console.error(e);
    }
          ...
          <motion.div className="modal-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div className="modal-content" initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }}>
              <div className="modal-header">
                <h2 className="modal-title">{editingId ? "Edit Data" : "Tambah Data"}</h2>
