...
const CSS = `
  ...
  .badge-category { background: #dcfce7; color: #166534; padding: 4px 10px; border-radius: 9999px; font-size: 12px; font-weight: 600; display: inline-block; }
...
export default function AdminPotensiAlamPage() {
  const [items, setItems] = useState<DataPotensiAlam[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [formData, setFormData] = useState({ kategori: "", jumlah: 0 });

  const fetchData = async () => {
    try {
      const res = await fetch("/api/data-potensi-alam");
  ...
  const openCreateModal = () => {
    setFormData({ kategori: "", jumlah: 0 });
    setEditingId(null);
    setIsModalOpen(true);
  };

  const openEditModal = (item: DataPotensiAlam) => {
    setFormData({ kategori: item.kategori, jumlah: item.jumlah });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const url = editingId ? `/api/data-potensi-alam/${editingId}` : "/api/data-potensi-alam";
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Hapus data kategori potensi alam ini?")) return;
    try {
      const res = await fetch(`/api/data-potensi-alam/${id}`, { method: "DELETE" });
      if (res.ok) fetchData();
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <div className="admin-page">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">Potensi Alam</h1>
          <p className="page-subtitle">Kelola data potensi alam berdasarkan kategori.</p>
        </div>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={4} style={{ textAlign: "center", padding: "48px 0", color: "#9ca3af" }}>
                    Belum ada data potensi alam.
                  </td>
                </tr>
              ) : (
                items.map(item => {
                  const percentage = totalJumlah > 0 ? ((item.jumlah / totalJumlah) * 100).toFixed(2) : "0.00";
                  return (
                    <tr key={item.id}>
                      <td style={{ fontWeight: 600 }}>{item.kategori}</td>
                      <td>{item.jumlah}</td>
                      <td>
                        <span className="badge-category">{percentage}%</span>
                      </td>
                      <td>
              ...
              <form onSubmit={handleSubmit}>
                <div className="modal-body">
                  <div className="form-group">
                    <label className="form-label">Kategori (Misal: Talago, Danau)</label>
                    <input 
                      type="text" className="form-input" placeholder="Misal: Talago"
