...
const CSS = `
  ...
  .table-container { background: #fff; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -2px rgba(0,0,0,0.05); border: 1px solid #e5e7eb; overflow: hidden; overflow-x: auto; }
  .filament-table { width: 100%; border-collapse: collapse; text-align: left; font-size: 14px; min-width: 800px; }
  .filament-table th { background: #f9fafb; padding: 12px 16px; font-weight: 600; color: #374151; border-bottom: 1px solid #e5e7eb; text-transform: uppercase; font-size: 12px; letter-spacing: 0.05em; }
  .filament-table td { padding: 16px; border-bottom: 1px solid #f3f4f6; color: #4b5563; vertical-align: middle; }
  .filament-table tr:last-child td { border-bottom: none; }
  .filament-table tr:hover { background: #f9fafb; }
  .badge-percent { background: #e0e7ff; color: #4338ca; padding: 4px 10px; border-radius: 9999px; font-size: 12px; font-weight: 600; display: inline-block; white-space: nowrap; }
  ...
  .modal-overlay { position: fixed; inset: 0; background: rgba(11, 31, 24, 0.4); backdrop-filter: blur(8px); z-index: 100; display: flex; align-items: center; justify-content: center; padding: 24px; overflow-y: auto; }
  .modal-content { background: #fff; border-radius: 24px; width: 100%; max-width: 600px; position: relative; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25); margin: auto; }
  ...
  .modal-body { padding: 32px; display: grid; gap: 16px; grid-template-columns: 1fr 1fr; }
...
export default function AdminSanitasiPage() {
  const [items, setItems] = useState<DataSanitasi[]>([]);
  ...
  const fetchData = async () => {
    try {
      const res = await fetch("/api/data-sanitasi");
  ...
  const openEditModal = (item: DataSanitasi) => {
    setFormData({ 
      jorong: item.jorong, 
      sampahMS: item.sampahMS, 
      spalMS: item.spalMS,
      jambanSehat: item.jambanSehat,
      aksesAir: item.aksesAir
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const url = editingId ? `/api/data-sanitasi/${editingId}` : "/api/data-sanitasi";
  ...
  return (
    <div className="admin-page">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">Data Sanitasi</h1>
          <p className="page-subtitle">Kelola data ketersediaan sumber air dan sanitasi warga.</p>
        </div>
      </div>
        ...
        <div className="table-container">
          <table className="filament-table">
            <thead>
              <tr>
                <th>Jorong</th>
                <th>% Tempat Sampah MS</th>
                <th>% Rumah Punya SPAL MS</th>
                <th>% Jamban Sehat</th>
                <th>% Periode ODG Akses Berkelanjutan & TDA Air Minum Berkelanjutan</th>
                ...
                items.map(item => {
                  return (
                    <tr key={item.id}>
                      ...
                      <td><span className="badge-percent">{item.sampahMS}%</span></td>
                      <td><span className="badge-percent">{item.spalMS}%</span></td>
                      <td><span className="badge-percent">{item.jambanSehat}%</span></td>
                      <td><span className="badge-percent">{item.aksesAir}%</span></td>
                      <td>
              ...
              <form onSubmit={handleSubmit}>
                <div className="modal-body">
                  ...
                  <div className="form-group">
                    <label className="form-label">% Tempat Sampah MS</label>
                    <input 
                      type="number" className="form-input" min="0" step="0.01" placeholder="0"
                      value={formData.sampahMS} onChange={e => setFormData({...formData, sampahMS: parseFloat(e.target.value) || 0})} required 
                    />
                  </div>
                  
                  <div className="form-group">
                    <label className="form-label">% Rumah Punya SPAL MS</label>
                    <input 
                      type="number" className="form-input" min="0" step="0.01" placeholder="0"
                      value={formData.spalMS} onChange={e => setFormData({...formData, spalMS: parseFloat(e.target.value) || 0})} required 
                    />
                  </div>

                  <div className="form-group">
                    <label className="form-label">% Jamban Sehat</label>
                    <input 
                      type="number" className="form-input" min="0" step="0.01" placeholder="0"
                      value={formData.jambanSehat} onChange={e => setFormData({...formData, jambanSehat: parseFloat(e.target.value) || 0})} required 
                    />
                  </div>

                  <div className="form-group">
                    <label className="form-label">% Akses Air Minum</label>
                    <input 
                      type="number" className="form-input" min="0" step="0.01" placeholder="0"
                      value={formData.aksesAir} onChange={e => setFormData({...formData, aksesAir: parseFloat(e.target.value) || 0})} required 
                    />
                  </div>
                </div>
