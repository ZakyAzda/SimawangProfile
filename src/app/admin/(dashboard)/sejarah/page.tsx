...
const CSS = `
  ...
  .form-input { width: 100%; padding: 12px 16px; border-radius: 10px; border: 1px solid #d1d5db; font-size: 14px; transition: border-color 0.2s, box-shadow 0.2s; background: #fff; }
...
export default function AdminSejarahPage() {
  const [items, setItems] = useState<DataSejarah[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [formData, setFormData] = useState({ jorong: JORONG_LIST[0], namaTempat: "" });

  const fetchData = async () => {
    try {
      const res = await fetch("/api/data-sejarah-budaya");
  ...
  const openCreateModal = () => {
    setFormData({ jorong: JORONG_LIST[0], namaTempat: "" });
    setEditingId(null);
    setIsModalOpen(true);
  };

  const openEditModal = (item: DataSejarah) => {
    setFormData({ jorong: item.jorong, namaTempat: item.namaTempat });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const url = editingId ? `/api/data-sejarah-budaya/${editingId}` : "/api/data-sejarah-budaya";
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Hapus situs sejarah ini?")) return;
    try {
      const res = await fetch(`/api/data-sejarah-budaya/${id}`, { method: "DELETE" });
      if (res.ok) fetchData();
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <div className="admin-page">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">Sejarah & Budaya</h1>
          <p className="page-subtitle">Inventaris data situs sejarah dan cagar budaya nagari.</p>
        </div>
        <button className="btn-primary" onClick={openCreateModal}>
          <Plus size={18} /> Tambah Situs
        </button>
      </div>
        ...
        <div className="table-container">
          <table className="filament-table">
            <thead>
              <tr>
                <th>Jorong</th>
                <th>Nama Tempat / Situs Cagar Budaya</th>
            ...
            <tbody>
              {items.length === 0 ? (
                <tr>
                  <td colSpan={3} style={{ textAlign: "center", padding: "48px 0", color: "#9ca3af" }}>
                    Belum ada data sejarah & budaya.
                  </td>
                </tr>
              ) : (
                items.map(item => (
                  <tr key={item.id}>
                    <td style={{ fontWeight: 600, width: "25%" }}>{item.jorong}</td>
                    <td>{item.namaTempat}</td>
                    <td style={{ width: "120px" }}>
                      <div className="table-actions">
                        <button className="action-icon-btn edit" onClick={() => openEditModal(item)}><Pencil size={18} /></button>
                        <button className="action-icon-btn delete" onClick={() => handleDelete(item.id)}><Trash2 size={18} /></button>
                      </div>
                    </td>
                  </tr>
                ))
          ...
          <motion.div className="modal-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div className="modal-content" initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }}>
              <div className="modal-header">
                <h2 className="modal-title">{editingId ? "Edit Situs" : "Tambah Situs"}</h2>
              ...
              <form onSubmit={handleSubmit}>
                <div className="modal-body">
                  ...
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    <label className="form-label">Nama Tempat / Situs</label>
                    <input 
                      type="text" className="form-input" placeholder="Misal: Medan Nan Bapaneh"
                      value={formData.namaTempat} onChange={e => setFormData({...formData, namaTempat: e.target.value})} required 
                    />
                  </div>
                </div>
