...
export default function AdminUMKMPage() {
  const [items, setItems] = useState<DataUmkm[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const url = editingId ? `/api/data-umkm/${editingId}` : "/api/data-umkm";
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/data-umkm/${id}`, { method: "DELETE" });
      if (res.ok) fetchData();
    } catch (e) {
      console.error(e);
    }
    ...
    <div className="admin-page">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Kelola data pelaku usaha dan produk lokal di nagari.</p>
        </div>
              ...
              <form onSubmit={handleSubmit}>
                <div className="modal-body">
                  <div className="form-group">
                    ...
                    <input 
                      type="text" className="form-input" placeholder="Misal: Kerajinan Bambu"
                      value={formData.productUmkm} onChange={e => setFormData({...formData, productUmkm: e.target.value})} required 
                    />
                  </div>
